...
const MediaVariants = ({media, messages}: MediaVariantsProps) => {
    ...
    return (
        <div>
            <div className="flex gap-4 flex-col">
                <VariantDropZone key="variant-drop-zone" callback={addVariant} messages={messages} media={media}
                                 localeId={locale}/>
            ...
            {media.variants.length > 0 ? (
                ...
                <div className="font-medium text-center">{messages["There are no loaded variants"]}</div>
            )}
        </div>
    )
}
