...
import type { QuerySortDirection } from "../../lib/query-builder/ModernQueryBuilder";
...
const resolveSortDirection = (value: string | undefined): QuerySortDirection | undefined => {
  if (!value) {
    return undefined;
  }
  return value.toLowerCase() === "asc" ? "ASC" : value.toLowerCase() === "desc" ? "DESC" : undefined;
};
...
export default async function publicApiData(req: ReqType, res: ResType) {
  try {
    ...
    const limitBase = normalizePositiveInt(req.query.limit, DEFAULT_LIMIT, MAX_LIMIT);
    const limit = format === "json" ? limitBase : Math.min(limitBase, FEED_LIMIT);
    const sort = getQueryStringValue(req.query.sort);
    const sortDirection = resolveSortDirection(
      getQueryStringValue(req.query.direction ?? req.query.sortDirection)
    );
    const globalSearch = getQueryStringValue(req.query.globalSearch);
